...
namespace Fuse.Android
{
	...
	extern (Android) internal class StaticLayout
	{
		...
		@{
			...
			android.text.StaticLayout.Builder builder = android.text.StaticLayout.Builder.obtain(text, 0, 0, paint, width);
			return builder.setAlignment(alignment).setIncludePad(includePad).setLineSpacing(spacingAdd, spacingMult).setMaxLines(maxLines == 0 ? Integer.MAX_VALUE : maxLines).setEllipsize(android.text.TextUtils.TruncateAt.END).build();
		@}
